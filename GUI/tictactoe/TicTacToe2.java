...
class TicTacToe2 implements ActionListener,MouseListener,MouseMotionListener	{
	...
	TicTacToe2()	{
		...
		Frame f = new Frame("Tic Tac TOe 2 "); 
		f.setSize(390,390);
	...
	cv.setEnabled(false);
		try {
		System.out.println("x: "+is.readUTF());
		x=Integer.parseInt(is.readUTF());
		System.out.println("y: "+is.readUTF());
	...
	//cv.setEnabled(false);
		try {
		System.out.println("x:= "+is.readUTF());
		int x=Integer.parseInt(is.readUTF());
		System.out.println("y:= "+is.readUTF());
		int y=Integer.parseInt(is.readUTF());
